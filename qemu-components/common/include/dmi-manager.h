...
public:
     ...
     */
    class DmiRegion : public qemu::DmiRegionBase
    {
    public:
        using Key = uintptr_t;
     ...
     */
    class DmiRegionAlias : public qemu::DmiRegionBase
    {
    private:
        uint64_t m_start;
        uint64_t m_end;
    ...
    public:
        ...
        {
            SCP_INFO("DMI.Libqbox") << "Creating Alias " << *this;
    ...
    void get_region(const tlm::tlm_dmi& info, int fd = -1)
    {
        ...
        if (existing_it != m_regions.end()) {
            if (existing_it->second.get_size() != size) {
                ...
                ("Overlapping DMI regions old size {:x} new size {:x}!", existing_it->second.get_size(), size);
